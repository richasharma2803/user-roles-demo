...
import { FormErrorsType, UserDataType } from "../../Types";

type SelcetesOptionType = {
	[key:number]: any
	hobby: string
}
...
export function UserAddEdit() {
  ...
  const [selectedOptions, setSelectedOptions] = useState<SelcetesOptionType[]>([]);
  ...
  const [formErrors, setFormErrors] = useState<FormErrorsType>({
	...
	const handleInputChange = (event: ChangeEvent<HTMLInputElement|HTMLTextAreaElement>| FormEvent<HTMLLIElement|HTMLButtonElement> | React.MouseEvent<HTMLInputElement>) => {
    const { name, value, type, checked, files } = event.target as { name: string; value: string; files?: FileList; type?:string; checked?: boolean };
    ...
    if(name == "hobbies")
    {
			if (formData[name].includes(value)) {
				let hobbies = formData[name].filter((hobby) => hobby !== value);
        setFormData({
          ...formData,
          [name]: hobbies,
        });
	...
	const handleRoleDropdown = (value: any) => 
  {
    ...
    try {
      const { data } = await UserService.getUser(userId || 0)
      if (data.status) {
        setFormData({
  ...
  const showSelectedHobbies = (value: string) => {
    ...
    if (Object.keys(errors).length == 0) {
      try {
        ...
        Data.append("gender", formData.gender)
				formData.hobbies.forEach((hobby: string) => {
          Data.append("hobbies[]", hobby);
        })
        Data.append("image", formData.profile[0])
        if(typeof(formData.doc) != "string" && formData.doc && formData.doc.length > 0)
        {
        ...
        if (data.status) {
          ...
          navigate('/users')
          if(currentUser && currentUser.id == data.data.user.id)
          {
				...
				<Card className="w-96 mt-4 w-full">
					...
					<CardBody className="flex flex-col gap-4">
						<div className="gird-cols-1 mb-12 grid gap-12 px-4 lg:grid-cols-2 xl:grid-cols-3">
							<div>
								<Input type="text" name="first_name" label="First Name" value={formData.first_name} onChange={handleInputChange} size="lg" crossOrigin={undefined} />
								<Error error={formErrors.first_name} />
							</div>

							<div>
								<Input type="text" name="last_name" label="Last Name" value={formData.last_name} onChange={handleInputChange} size="lg" crossOrigin={undefined} />
								...
								<div className="relative flex w-full">
									<Menu placement="bottom-start">
										...
										<MenuList className="max-h-[20rem] max-w-[18rem]">
											{codes.map(({ country, countryCodes }: any) => {
												return (
													<MenuItem
														name="country_code"
														key={country}
														...
														// onChange={() => setCountryCode(countryCodes[0])}
													>
							...
							<div>
								<Input type="email" name="email" label="Email" value={formData.email} onChange={handleInputChange} size="lg" crossOrigin={undefined} />
								<Error error={formErrors.email} />
							</div>

							<div>
								<Input type="password" name="password" label="Password" onChange={handleInputChange} size="lg" crossOrigin={undefined} />
							...
							<div>
								<Input type="date" name="dob" label="date of Birth" value={formData.dob} onChange={handleInputChange} size="lg" crossOrigin={undefined} />
								<Error error={formErrors.dob} />
							</div>

							<div>
								<Textarea name="address" label="Address" value={formData.address} onChange={(event) => handleInputChange(event)} size="lg" />
						...
						<div className="gird-cols-1 mb-12 grid gap-12 px-4 lg:grid-cols-2 xl:grid-cols-2">
							<div className="flex flext-col gap-1">
									...
									{gender.map((option) => (
										<div key={option.value}>
											<Radio name="gender" value={option.value} label={option.label} checked={formData.gender.includes(option.value)} onChange={handleInputChange} crossOrigin={undefined} />
										</div>
									))}
									<Typography
										variant="h6"
							...
							<div className="flex flext-col gap-1">
								...
								{hobbies.map((option) => (
										<div key={option.value}>
											<Checkbox name="hobbies" value={option.value} label={option.label} defaultChecked={showSelectedHobbies(option.value)} onClick={(event) => handleInputChange(event)} crossOrigin={undefined} />
										</div>
									))}
								</div>
						...
						<div className="gird-cols-1 mb-12 grid gap-12 px-4 lg:grid-cols-2 xl:grid-cols-3">
								<div>
									<Input type="file" name="profile" accept="image/*" label="Profile Picture" onChange={handleInputChange} size="lg" crossOrigin={undefined} />
									<Error error={formErrors.profile} />
								</div>
							<div>
							<div>
									<Input type="file" name="doc" accept=".pdf" label="ID proofs" onChange={handleInputChange} size="lg" multiple crossOrigin={undefined} />
