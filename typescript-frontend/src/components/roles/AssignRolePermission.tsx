import { Card, CardBody, CardHeader, Checkbox, Typography } from "@material-tailwind/react";
...
export function AssignRolePermission() {
    ...
    const [selectedPermission, setSelectedPermission] = useState<NewDataType[]>([]);

    const handlePermissionChange = async(roleId:string|number|undefined, permission: string) => {
        console.log(roleId, permission);
  
        try {
          const Data = new FormData();
          Data.append('permission', permission); 
          const { data } = await RoleService.assignRolePermission(roleId || 0, Data);
          if (data.status) {
        ...
        try {
          ...
          if (data.status) {
            ...
            {role_permissions.map(
              (role_permission: Role, key: number) => {
                newData[key] = {
    ...
    const showSelectedPermissions = (id: number, value: string) => {
                            ...
                            return (
                                ...
                                {roles.map(({id}) => (
                                    ...
                                    <Checkbox 
                                        ...
                                        onChange={() => handlePermissionChange(id, permission)} crossOrigin={undefined}                                    />
                                    </div>
                                </td>
                                ))}
                                </tr>
